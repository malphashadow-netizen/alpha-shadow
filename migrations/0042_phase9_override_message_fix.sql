-- Migration 0042 — Phase 9 (message fix): truthful override-evidence messages.
--
-- Block (f) of validate_stock_movement() (override evidence: attempt validity,
-- freshness, single-use claim) runs for ANY row citing manager_override_id,
-- including waste_void / waste_refund rows — but its three messages were
-- hard-coded with a 'sale_deduction ...' prefix, misleading any future
-- security reviewer into believing the gate is sale-only. This migration
-- re-emits those three messages with the ACTUAL movement type (a bare %
-- placeholder + NEW.movement_type, the same RAISE style 0040 already uses). For sale_deduction rows the text is byte-identical to
-- 0040; for waste rows it now names the real type.
--
-- MESSAGE TEXT ONLY: no table, column, constraint, index, trigger, grant, or
-- logic change. In particular the sale-only negative-balance gate — including
-- the stable 'stock: insufficient quantity' prefix pinned by the orders-store
-- classifier (Decision 9) — is byte-identical. 0040 itself is untouched.
--
...
BEGIN
  ...
  IF NEW.manager_override_id IS NOT NULL THEN
    ...
    IF v_attempt.outcome IS DISTINCT FROM 'succeeded'
      ...
      RAISE EXCEPTION '% override requires a successful stock_override attempt bound to the same actor', NEW.movement_type USING ERRCODE = '23514';
    END IF;
