...
BEGIN
  ...
  ELSE
    FOR v_layer IN SELECT * FROM inventory_cost_layers
      ...
      ORDER BY is_provisional, created_at, id FOR UPDATE
    LOOP
    ...
    IF v_needed_units > 0 THEN
      -- Phase 8 forbids provisional-to-provisional valuation chains. A
      -- shortfall may inherit cost only from the latest real receipt, regardless
      -- of newer provisional layers.
      SELECT l.total_cost_minor, (l.original_qty * 10000)::bigint, l.currency_code, l.minor_unit_digits
        INTO v_basis_cost, v_basis_units, v_currency, v_digits
        FROM inventory_cost_ledger l
        JOIN stock_movements sm ON sm.tenant_id = l.tenant_id AND sm.id = l.stock_movement_id
...
BEGIN
  ...
  IF NEW.movement_type = 'manual_adjustment' AND NEW.adjustment_reason_id IS NULL THEN
    RAISE EXCEPTION 'manual_adjustment requires an adjustment reason' USING ERRCODE = '23514';
  END IF;
