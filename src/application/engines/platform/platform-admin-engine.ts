...
import type { PlatformAdminRepository } from '../../../domain/contracts/platform-admin.ts';
...
export class PlatformAdminEngine {
  ...
  async listTenants(actor: PlatformActor) { await this.authorize(actor, 'platform:tenant:list'); return this.repository.listTenants(PLATFORM_TENANT_ID); }
  async getTenantDetails(actor: PlatformActor, tenantId: string) { await this.authorize(actor, 'platform:tenant:view'); this.rejectPlatform(tenantId); const tenant = await this.repository.getTenant(tenantId); if (tenant === null) throw new NotFoundError(`tenant ${tenantId} not found`); return tenant; }
  async assignSubscriptionPlan(actor: PlatformActor, tenantId: string, planId: string): Promise<void> { await this.authorize(actor, 'platform:subscription:assign'); this.rejectPlatform(tenantId); const plan = await this.plans.getPlanById(planId); if (plan === null || !plan.isActive) throw new NotFoundError(`active subscription plan ${planId} not found`); const startsAt = this.now(); const endsAt = new Date(startsAt.getTime() + plan.durationDays * 86_400_000); await this.repository.assignSubscription(tenantId, plan, startsAt, endsAt); }
}
