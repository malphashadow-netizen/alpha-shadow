...
function validate(input: Partial<CreateSubscriptionPlanInput | UpdateSubscriptionPlanInput>): void {
  if (input.name?.trim() === '') throw new ValidationError('name must be non-empty', 'name');
...
export class SubscriptionPlansEngine {
  ...
  async createPlan(actor: PlatformActor, input: CreateSubscriptionPlanInput): Promise<SubscriptionPlan> { await this.authorize(actor); validate(input); return this.repository.createPlan(input); }
  async updatePlan(actor: PlatformActor, id: string, input: UpdateSubscriptionPlanInput): Promise<SubscriptionPlan> { await this.authorize(actor); validate(input); return this.repository.updatePlan(id, input); }
  async deactivatePlan(actor: PlatformActor, id: string): Promise<void> { await this.authorize(actor); return this.repository.deactivatePlan(id); }
  async listPlans(actor: PlatformActor, includeInactive = false): Promise<readonly SubscriptionPlan[]> { await this.authorize(actor); return this.repository.listPlans(includeInactive); }
  async getPlanById(actor: PlatformActor, id: string): Promise<SubscriptionPlan | null> { await this.authorize(actor); return this.repository.getPlanById(id); }
}
