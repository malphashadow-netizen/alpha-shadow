...
export class InMemoryPlatformAdminRepository implements PlatformAdminRepository {
  ...
  async createTenant(id: string, name: string): Promise<void> { this.tenants.set(id, { id, name, status: 'active', subscriptionPlanId: null, subscriptionStatus: 'active', subscriptionStartedAt: null, subscriptionEndsAt: null }); this.permissionStore?.tenants.set(id, { id, name, status: 'active' }); }
