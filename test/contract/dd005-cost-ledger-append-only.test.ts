...
async function fixture(
  ...
  await client.query(
    ...
    [movement, tenant, branch, item, user],
  );
  const ledger = await client.query<{ readonly id: string }>(
    "INSERT INTO inventory_cost_ledger (tenant_id,inventory_item_id,stock_movement_id,total_cost_minor,original_qty,currency_code,minor_unit_digits) VALUES ($1,$2,$3,100,10,$4,2) RETURNING id",
    [tenant, item, movement, "SAR"],
  );
  const layer = await client.query<{ readonly id: string }>(
    "INSERT INTO inventory_cost_layers (tenant_id,inventory_item_id,cost_ledger_id,original_qty,remaining_qty,total_cost_minor,remaining_cost_minor,currency_code,minor_unit_digits) VALUES ($1,$2,$3,10,10,100,100,$4,2) RETURNING id",
    [tenant, item, ledger.rows[0]?.id, "SAR"],
  );
  return {
    ledger: Number(ledger.rows[0]?.id),
    layer: Number(layer.rows[0]?.id),
    branch,
    item,
    user,
  };
...
async function tx(work: (client: pg.Client) => Promise<void>): Promise<void> {
...
describe("DD-005 cost ledger append-only barriers", () => {
  ...
  it("blocks UPDATE on inventory_cost_ledger", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query(
            "UPDATE inventory_cost_ledger SET total_cost_minor=99 WHERE id=$1",
            [f.ledger],
          ),
        "55006",
      );
    }));
  it("blocks DELETE on inventory_cost_ledger", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query("DELETE FROM inventory_cost_ledger WHERE id=$1", [
            f.ledger,
          ]),
        "55006",
      );
    }));
  it("blocks DELETE on inventory_cost_layers", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query("DELETE FROM inventory_cost_layers WHERE id=$1", [
            f.layer,
          ]),
        "55006",
      );
    }));
  it("blocks changing an immutable layer column", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query(
            "UPDATE inventory_cost_layers SET tenant_id=$1, original_qty=9, total_cost_minor=99 WHERE id=$2",
            [tenant, f.layer],
          ),
        "42501",
      );
    }));
  it("blocks increasing remaining_qty", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query(
            "UPDATE inventory_cost_layers SET remaining_qty=11 WHERE id=$1",
            [f.layer],
          ),
        "42501",
      );
    }));
  it("blocks increasing remaining_cost_minor", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      await rejected(
        () =>
          client.query(
            "UPDATE inventory_cost_layers SET remaining_cost_minor=101 WHERE id=$1",
            [f.layer],
          ),
        "42501",
      );
    }));
  it("allows decreasing remaining_qty and remaining_cost_minor together", async () =>
    tx(async (client) => {
      const f = await fixture(client);
      ...
      const row = await client.query<{
        ...
        "SELECT remaining_qty::text,remaining_cost_minor::text FROM inventory_cost_layers WHERE id=$1",
        [f.layer],
      );
  ...
  it("grants app_login insert and select but never update on the ledger", async () => {
    const owner = await connectTestClient();
    let f: Awaited<ReturnType<typeof fixture>>;
    try {
      f = await fixture(owner);
      const movement = randomUUID();
      await owner.query(
        "INSERT INTO stock_movements (id,tenant_id,branch_id,inventory_item_id,movement_type,quantity_delta,actor_user_id) VALUES ($1,$2,$3,$4,'manual_receiving',10,$5)",
        [movement, tenant, f.branch, f.item, f.user],
      );
      ...
      try {
        ...
        await rejected(
          () =>
            client.query("UPDATE inventory_cost_ledger SET total_cost_minor=1"),
          "42501",
        );
        await rejected(
